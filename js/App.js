...
async function sendHash() {
  ...
  if (!file) {
    ...
    const node = await Ipfs.create({ repo: "Ali-ok" + Math.random() });

    const fileReader = new FileReader();
    fileReader.onload = async (event) => {
      try {
        const result = await node.add(event.target.result);
        window.ipfsCid = result.path;
        const MyCID = window.ipfsCid + "/";
        console.log("My-CID: " + MyCID);
      } catch (error) {
...
function generateQRCode() {
  ...
  let url = `${window.location.host}/blockchain/verify.html?hash=${window.hashedfile}`;
  qrcode.makeCode(url);
...
//   var userImage = document.getElementById("1");
//   var hangoutButton = document.getElementById("2");
