window.CONTRACT = {
  address: "0xEeA99dD5d1A224BF0CB3412B548dDdDc85b1157A",
...
function checkURL() {
  ...
  console.log(url.searchParams.get("hash"), "sjhd");
  verify_Hash();
}
...
async function get_Sha3() {
  ...
  if (file) {
    ...
    reader.onload = async function (evt) {
      ...
      $("#note").html(
        `<h5 class="text-center text-info">Document Hashed11  😎  </h5>`
      );
    };
...
async function print_info(result, is_verified) {
  ...
  if (!is_verified) {
    ...
    var file = await document.getElementById("doc-file").files[0];
    var img = document.getElementById("student-document");
    var reader = new FileReader();
    ...
    document.getElementById("student-document").src =
      "https://sepolia.etherscan.io/" + result[3];
